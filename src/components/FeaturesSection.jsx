...
const CAPABILITIES = [
  {
    ...
    description:
      'AMRs communicate directly through decentralized peer-to-peer coordination, exchanging spatial telemetry and peer state without relying on a central communication broker.',
    image: '/features/p2p-mesh.jpeg',
    accentColor: '#38bdf8',
    icon: Radio,
    telemetry: [
      { label: 'TOPOLOGY', val: 'DECENTRALIZED P2P MESH' },
      { label: 'HEARTBEAT', val: '10 HZ PEER STATE' },
      { label: 'PEER TIMEOUT', val: '3.0 S STALE STATE' },
    ],
  },
  {
    id: 'blocked-aisle',
    ...
    description:
      'Dynamic obstacles trigger peer alerts and local A* replanning, allowing each AMR to recalculate a new route around blocked cells while preserving the shared warehouse state.',
    image: '/features/blocked-aisle.jpeg',
    accentColor: '#f87171',
    icon: Radar,
    telemetry: [
      { label: 'DETECTION', val: 'DYNAMIC OBSTACLE' },
      { label: 'PLANNING', val: 'LOCAL A* REPLAN' },
      { label: 'MOVEMENT', val: '4-CONNECTED GRID' },
    ],
  },
  {
    id: 'deadlock',
    ...
    tag: 'SYSTEM 03 // DEADLOCK RESOLUTION',
    title: 'Spatiotemporal Deadlock Elimination',
    shortTitle: 'Deadlock Elimination',
    description:
      'When AMRs form circular wait dependencies, the system detects deadlock cycles using a wait-for graph and DFS, then resolves them through priority scoring and safe-yield cell selection.',
    image: '/features/deadlock.jpeg',
    accentColor: '#38bdf8',
    icon: Compass,
    telemetry: [
      { label: 'DETECTION', val: 'WAIT-FOR GRAPH + DFS' },
      { label: 'RESOLUTION', val: 'PRIORITY SCORING' },
      { label: 'RECOVERY', val: 'BFS SAFE YIELD CELL' },
    ],
  },
];
